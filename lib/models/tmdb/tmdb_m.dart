...
class ImageObject with _$ImageObject {
  const factory ImageObject({
    ...
    @Default(0) int width,
    @JsonKey(name: 'file_type') String? fileType, // Specific to Company Images
    @Default('') String id, // Specific to ChangeItem Image Value
  }) = _ImageObject;
...
class TranslationData with _$TranslationData {
  const factory TranslationData({
    String? homepage,
    String? overview,
    int? runtime, // Can be int or string depending on context
    String? tagline,
    String? title,
...
class MediaAccountStates with _$MediaAccountStates {
  const factory MediaAccountStates({
    ...
    dynamic rated, // Can be bool false or an object {value: double}. Use dynamic and parse manually if needed
...
class ChangeDetail with _$ChangeDetail {
  const factory ChangeDetail({
    ...
    @JsonKey(name: 'value') dynamic changeValue, // Can be String, Map, etc.
...
class CreditMedia with _$CreditMedia {
  const factory CreditMedia({
    ...
    @Default(0.0) @JsonKey(name: 'vote_average') double voteAverage,
    @Default(0) int? voteCount, // Can be null in some examples
...
class MovieResultWithRating  with _$MovieResultWithRating {
  const factory MovieResultWithRating({
    ...
    double? rating, // Specific to rated items
...
class EpisodeAccountState with _$EpisodeAccountState {
  const factory EpisodeAccountState({
    @Default(0) int id,
    @Default(0) @JsonKey(name: 'episode_number') int episodeNumber,
    dynamic rated, // Can be bool false or object. Use dynamic and parse manually if needed
...
class PersonCombinedCredits with _$PersonCombinedCredits {
  const factory PersonCombinedCredits({
    @Default([]) List<dynamic> cast, // Can be PersonMovieCreditItem or PersonTvCreditItem. Use dynamic and parse based on media_type
    @Default([]) List<dynamic> crew, // Can be PersonMovieCreditItem or PersonTvCreditItem. Use dynamic and parse based on media_type
    @Default(0) int id,
