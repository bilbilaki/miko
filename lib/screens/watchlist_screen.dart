...
class WatchlistScreen extends StatelessWidget {
 ...
 var userData = Provider.of<UserDataService>(context);
    ...
    TvSeriesProvider tvProvider = Provider.of<TvSeriesProvider>(context, listen: false);
    AnimeProvider animeProvider = Provider.of<AnimeProvider>(context, listen: false);
        List<Movie>  watchlistMovies = userData.watchlistMovieIds
        .map((id) => movieProvider.getMovieById(id))
        .whereType<Movie>()
        .toList();

     List<TvSeriesAnime>  watchlistTvSeries = userData.watchlistTvSeriesIds
        .map((id) => tvProvider.getAnimeByTmdbId(id))
          .whereType<TvSeriesAnime>()
          .toList();

       List<TvSeriesAnime> watchlistAnime = userData.watchlistAnimeIds
        .map((id) => animeProvider.getAnimeByTmdbId(id))
        .whereType<TvSeriesAnime>() // Filter out nulls if movie not found
        .toList();

    List<Object> allWatchlist = [...watchlistMovies, ...watchlistTvSeries, ...watchlistAnime];
...
class FavoritesScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // Get favorite items
    List<Movie>  favoriteMovies = userData.favoriteMovieIds
        .map((id) => movieProvider.getMovieById(id))
        .whereType<Movie>() // Filter out nulls if movie not found
        .toList();
    List<TvSeriesAnime> favoriteAnime = userData.favoriteAnimeIds
        .map((id) => animeProvider.getAnimeByTmdbId(id))
        .whereType<TvSeriesAnime>() // Filter out nulls if movie not found
        .toList();

     List<TvSeriesAnime>  favoriteTvSeries = userData.favoriteAnimeIds
        .map((id) => tvProvider.getAnimeByTmdbId(id))
         .whereType<TvSeriesAnime>() // Filter out nulls
        .toList();

    // Combine and sort (optional, e.g., alphabetically)
    List<Object> allFavorites = [
      ...favoriteMovies,
      ...favoriteTvSeries,
      ...favoriteAnime
    ];
